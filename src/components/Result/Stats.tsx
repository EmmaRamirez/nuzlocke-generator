      ...
      <div className="stats stats-container" style={{ color }}>
        ...
        <div style={{ marginTop: '10px', margin: '0 10px' }}>
          ...
          {style.statsOptions.mostCommonKillers ? (
            <div>Most Common Killers: {this.displayMostCommonDeath(this.getMostCommonDeath())}</div>
          ) : null}
          {style.statsOptions.mostCommonTypes ? (
            <div>Most Common Types: {this.displayMostCommonType(this.getMostCommonType())}</div>
          ) : null}
          {style.statsOptions.shiniesCaught ? (
            <div>
              Shinies: <Layout display={LayoutDisplay.Inline}>{this.getShinies()}</Layout>
            </div>
          ) : null}
          {stats?.length
            ? stats?.map((stat, idx) =>
