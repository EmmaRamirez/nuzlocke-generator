 ...
 */
export const SaveButton = ({ saved = false, onClick }: SaveButtonProps) => (
    <button
        className={saved ? 'pt-button pt-intent-success' : 'pt-button'}
        onClick={e => {
            e.preventDefault();
            onClick();
        }}>
        {saved ? (
            <span>
                <span className='pt-icon-saved' /> Saved
            </span>
        ) : (
            <span>
                <span className='pt-icon-floppy-disk' /> Save
            </span>
        )}
    </button>
);
