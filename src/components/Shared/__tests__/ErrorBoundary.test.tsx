...
describe('<ErroBoundary />', () => {
    it('renders its children', () => {
        const wrapper = shallow(<ErrorBoundary><div>Test</div></ErrorBoundary>);
        expect(wrapper.contains(<div>Test</div>)).toBe(true);
    });

    it('catches errors and renders errorMessage', () => {
        const spy = sinon.spy(ErrorBoundary.prototype, 'componentDidCatch');
        const wrapper = mount(<ErrorBoundary errorMessage={<div>Error!</div>}><ErroneousComponent /></ErrorBoundary>);
