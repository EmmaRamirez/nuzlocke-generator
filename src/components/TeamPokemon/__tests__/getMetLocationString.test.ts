...
describe(getMetLocationString.name, () => {
    ...
    it('works with the new format', () => {
        ...
        expect(getMetLocationString({ poke, oldMetLocationFormat: false })).toEqual(`Met Location: Route 101 at lv.4`);
    });

    it ('works with the old format', () => {
        // `Met ${determinePreposition()} ${met}, from lv.${metLevel}`;
        expect(getMetLocationString({ poke, oldMetLocationFormat: true })).toEqual(`Met on Route 101, from lv.4`);
    });
